...
from tutor import get_tutor_reply, get_tutor_image_reply
from sheet_logger import log_interaction
from telegram_adapter import send_telegram_message, configure_telegram_webhook, download_telegram_image
...
def whatsapp_migration_mode_enabled() -> bool:
    return os.getenv("WHATSAPP_MIGRATION_MODE", "true").strip().lower() in {"1", "true", "yes", "on"}
...
def _validate_twilio_request(request: Request, form) -> None:
    ...
    if not RequestValidator(auth_token).validate(str(request.url), dict(form), signature):
...
async def whatsapp_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    status = "Success"
    try:
        reply_text, latency = get_tutor_reply(pilot_id, body)
    except Exception:
...
async def _handle_telegram_message(chat_id: int, text: str, pilot_id: str, school: str, session_id: str):
    status = "Success"
    try:
        reply_text, latency = get_tutor_reply(pilot_id, text)
    except Exception:
...
async def _handle_telegram_image(chat_id: int, file_id: str, caption: str, pilot_id: str, school: str, session_id: str):
    """Download an image transiently, teach from it, and log metadata/text only."""
    status = "Success"
    try:
        image_bytes, mime_type = await download_telegram_image(file_id)
        reply_text, latency = get_tutor_image_reply(pilot_id, image_bytes, mime_type, caption)
    except ValueError as exc:
        reply_text, latency, status = f"I couldn't use that image safely: {exc}. Please send a clear JPG, PNG, or WEBP photo of the Maths question.", 0.0, "RejectedImage"
    except Exception:
        logger.exception("Image tutoring failed (Telegram)")
        reply_text, latency, status = "Sorry, I couldn't read that image just now. Please try again with a clear photo or type the Maths question.", 0.0, "Error"
    await send_telegram_message(chat_id, reply_text)
    safe_log_message = "[Maths image received]" + (f" Caption: {caption[:180]}" if caption else "")
    log_interaction(pilot_id, school, "Telegram", session_id, safe_log_message, reply_text, latency, status)
...
async def telegram_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    document_mime = str(document.get("mime_type", ""))
    ...
    onboarding_text = text or ("Maths image" if image_file_id else "")
    if not onboarding_text:
        background_tasks.add_task(send_telegram_message, chat_id, "Send me a JSS2 Maths question, or send a clear photo of a Maths problem and I can help you work through it.")
        return {"ok": True}
    ...
    _, pilot_id, school, session_id = outcome
    if image_file_id:
        background_tasks.add_task(_handle_telegram_image, chat_id, image_file_id, text, pilot_id, school, session_id)
    else:
